...
- 호이스팅(hoisting)
  ...
  - 호이스팅(hoisting)은 JavaScript 엔진이 코드를 해석하는 과정에서 변수나 함수의 선언을 코드의 최상단으로 끌어올리는 현상. 그러나 호이스팅에서 중요한 점은 오직 선언만이 끌어올려진다는 것. 할당(초기화) 부분은 원래 코드에 있던 그 위치에서 그대로 수행됨.
  - var의 경우 변수 선언 이전의 위치에서 접근 시 undefined를 반환
